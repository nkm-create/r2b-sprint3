...
| 2 | 条件設定 | マスタ反映状況を確認し、ターム固有の調整と全体ポリシーを設定 | 条件の整理が難しい | マスタから自動プリセット、差分のみ設定 |
| 3 | 時間割生成 | AIエージェントが最適化アルゴリズムで時間割を生成 | 待ち時間がある | 進捗モニタリングで状況把握可能 |
...
| # | フェーズ | 行動 | 課題・感情 | 解決 |
|---|---------|------|----------|------|
| 2-1 | ターム選択 | 新規タームを作成または既存タームを選択 | どこから始めるか迷う | タームドロップダウンで明確に |
| 2-2 | マスタ確認 | 講師・生徒・教室設定の自動反映内容を確認 | 設定漏れがないか不安 | 件数とサマリーを表示 |
| 2-3 | ターム調整 | 今月だけの特別な調整を設定 | どこを変更すべきか迷う | 調整ボタンから個別設定 |
| 2-4 | ポリシー設定 | 志望レベルマッチング、充足率目標等を設定 | 最適な設定がわからない | デフォルト値を用意、調整可能 |
...
- **条件設定時**: 「マスタから自動で読み込まれている。差分だけ設定すればいい」（安心）
...
| 4 | 詳細確認 | 特定教室の詳細データを確認 | 深掘りに時間がかかる | ドリルダウンで詳細パネルに遷移 |
...
| # | フェーズ | 行動 | 課題・感情 | 解決 |
|---|---------|------|----------|------|
| 4-1 | 教室選択 | 一覧から問題のある教室をクリック | どの教室から見るべきか | アラートバッジで優先度表示 |
| 4-2 | 詳細表示 | 右パネルに教室詳細がスライドイン | 画面遷移が面倒 | 同一画面内でパネル表示 |
| 4-3 | データ確認 | 充足率推移、人員ヒートマップを確認 | データが見づらい | グラフ・ヒートマップで視覚化 |
| 4-4 | 閉じる | パネルを閉じて一覧に戻る | - | ×ボタンまたは一覧クリック |
